...
export function ResultsSection() {
  ...
  const marqueeItems = [...featuredResults, ...featuredResults, ...featuredResults];

  return (
    <section className="section bg-[var(--bg-secondary)] overflow-hidden py-12 md:py-16">
            ...
            <div className="overflow-hidden marquee-container py-4">
              <div className="flex gap-6 marquee-track" style={{ width: "max-content" }}>
                {marqueeItems.map((result, i) => (
                  <div
                    key={`${result.id}-${i}`}
                    className="flex-shrink-0 w-72 card p-6 cursor-default hover:border-[var(--brand-400)] transition-all duration-300 bg-[var(--bg-card)] shadow-md"
                  >
                    {/* Header */}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div
                        className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-wider"
