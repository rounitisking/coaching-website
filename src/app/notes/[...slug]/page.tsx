...
function resolvePathAndNames(segments: string[]): { resolvedPath: string; displayNames: string[] } | null {
  let current = path.join(process.cwd(), "public", "notes");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!resolved) return { title: "Notes Not Found" };
  ...
  return {
    title: `${breadcrumbTitle} — Notes | Academica Institute`,
    description: `Study materials, revision papers, and files for ${breadcrumbTitle} at Academica Institute.`,
  };
}

export default async function DynamicNotesPage({ params }: Props) {
  const { slug } = await params;
  ...
  // Read directories & files inside resolvedPath
  let items: fs.Dirent[] = [];
  try {
  ...
  // Separate subfolders and PDF files
  const folders = items
      ...
      <section className="section-padding" style={{ background: "var(--gradient-hero)" }}>
        <div className="container-custom">
          ...
          <p className="text-blue-100/70 text-sm md:text-base mt-2 max-w-2xl">
            {isLeafLevel ? "Available notes and study guides inside this folder." : "Select a folder below to explore chapters."}
          </p>
        </div>
      </section>
                ...
                {folders.map((folder) => {
                  ...
                  return (
                    <Link
                      key={folder}
                      href={linkPath}
                      className="group block card p-5 hover:-translate-y-0.5 hover:border-[#D4AF37]/40 hover:shadow-md transition-all duration-300"
