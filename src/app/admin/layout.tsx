...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-[var(--bg-secondary)] text-[var(--text-primary)]">
      {/* Sidebar */}
      <aside className="w-full lg:w-64 bg-[var(--bg-primary)] text-slate-400 border-r border-[var(--border)] flex flex-col justify-between shrink-0">
        <div className="p-6">
          ...
          <nav className="space-y-0.5 max-h-[70vh] overflow-y-auto">
            {links.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-400 hover:bg-[var(--bg-secondary)] hover:text-slate-100 transition-colors"
              >
                <link.icon size={16} />
                {link.label}
              </Link>
